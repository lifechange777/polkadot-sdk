...
pub mod pallet {
	use super::*;
	use frame_support::{
		pallet_prelude::{IsType, StorageDoubleMap, ValueQuery},
		Twox64Concat,
	};
		...
		VotingProofAccountIdOf<T, I>: From<T::AccountId>,
	{
		...
		fn try_vote(
			...
			// TODO: FAIL-CI - replace get total_balance from proof and check with local accounting for previous votes - with extend_lock
			...
			T::Polls::try_access_poll(poll_index, |poll_status| {
				...
				VotingFor::<T, I>::try_mutate(who.clone(), &class, |voting| {
					...
					// TODO: FAIL-CI do some local accounting accross proofs for accounts
