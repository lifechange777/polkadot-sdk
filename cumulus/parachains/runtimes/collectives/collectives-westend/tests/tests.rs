...
use pallet_proofs_voting::{AccountVote, Conviction, Vote};
...
fn dday_feature_works() {
	sp_io::TestExternalities::new(Default::default()).execute_with(|| {
		...
		let valid_account = AccountId::from_ss58check(ss58_account).expect("valid accountId");
		let vote = AccountVote::Standard {
			vote: Vote { aye: true, conviction: Conviction::Locked1x },
			balance: 1,
		};
		assert_ok!(DDayVoting::vote(
			RuntimeOrigin::signed(valid_account),
			referenda_id,
			vote,
